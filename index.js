...
// Bảng màu cố định riêng biệt cho từng loại thông báo
const COLORS = {
    SUCCESS: 0x2ECC71, // Xanh lá
    ERROR: 0xE74C3C,   // Đỏ
    WARNING: 0xF39C12, // Cam
    INFO: 0x3498DB,    // Xanh dương
    PURPLE: 0x9B59B6,  // Tím (Dành cho Admin)
    GOLD: 0xF1C40F     // Vàng (Dành cho Key/Lấy Key)
};

// Hàm Footer chỉ trả về tên và icon, thời gian sẽ do setTimestamp() tự động xử lý chuẩn Discord
...
client.on('interactionCreate', async interaction => {
    ...
    try {
        if (commandName === 'setadmin') {
            if (userId !== OWNER_ID) {
                return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.ERROR).setTitle('❌ Từ Chối Truy Cập').setDescription('Chỉ Owner mới có quyền thực hiện lệnh này!').setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
            }
            ...
            if (action === 'add') {
                ...
                const embed = new EmbedBuilder().setColor(COLORS.PURPLE).setTitle('🛡️ Đã Thêm Admin').setDescription(`✅ Đã thêm thành công **${target.tag}** vào danh sách quản trị viên.`).setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp();
                interaction.editReply({ embeds: [embed] });
            } else {
                await Admin.deleteOne({ user_id: target.id });
                const embed = new EmbedBuilder().setColor(COLORS.PURPLE).setTitle('🛡️ Đã Xóa Admin').setDescription(`✅ Đã xóa **${target.tag}** khỏi danh sách quản trị viên.`).setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp();
                interaction.editReply({ embeds: [embed] });
            }
        } 
            ...
            if (!isAdmin && userId !== OWNER_ID) {
                return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.ERROR).setTitle('❌ Từ Chối Truy Cập').setDescription('Bạn không có quyền sử dụng lệnh này!').setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
            }
            ...
            const isAdmin = await Admin.findOne({ user_id: userId });
            if (!isAdmin && userId !== OWNER_ID) {
                return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.ERROR).setTitle('❌ Từ Chối').setDescription('Bạn không có quyền thực hiện lệnh này!').setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
            }
            const targetUser = interaction.options.getUser('user');
            ...
            if (targetUser) {
                ...
                try {
                    const dmEmbed = new EmbedBuilder()
                        ...
                        .setDescription(`\`${tokenStr}\``)
                        .setThumbnail(targetAvatar)
                        .setFooter(getFooterOptions())
                        .setTimestamp();
                    await targetUser.send({ embeds: [dmEmbed] });
                    
                    interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.INFO).setTitle('✅ Thành Công').setDescription(`Đã gửi token reset HWID tới **${targetUser.tag}** qua DM.\n\n\`${tokenStr}\``).setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
                } catch (e) {
                    interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.WARNING).setTitle('⚠️ Cảnh Báo').setDescription(`Không thể gửi DM cho user này.\n\n\`${tokenStr}\``).setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
                }
            } else {
                interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.INFO).setTitle('🔑 Token Reset HWID').setDescription(`\`${tokenStr}\``).setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
            }
        }
        else if (commandName === 'removekey') {
            if (userId !== OWNER_ID) {
                return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.ERROR).setTitle('❌ Từ Chối').setDescription('Chỉ Owner mới có quyền xóa key!').setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
            }
            const toolKey = interaction.options.getString('toolkey');
            const row = await Key.findOneAndDelete({ assigned_key: toolKey });
            
            if (!row) {
                return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.ERROR).setTitle('❌ Lỗi').setDescription(`Không tìm thấy key tool với mã: \`${toolKey}\``).setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
            }
            interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.ERROR).setTitle('🗑️ Đã Xóa Key').setDescription(`✅ Đã xóa vĩnh viễn key tool:\n\n\`${toolKey}\``).setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
        }
        else if (commandName === 'redeem') {
            const inputKey = interaction.options.getString('key');
            const row = await Key.findOne({ key: inputKey });
            
            if (!row) return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.ERROR).setTitle('❌ Thất Bại').setDescription('Mã key không tồn tại trong hệ thống!').setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
            if (row.is_used === 1 || row.assigned_key) return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.WARNING).setTitle('⚠️ Thất Bại').setDescription('Mã key này đã được kích hoạt trước đó!').setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
            if (row.expires_at !== 0 && Date.now() > row.expires_at) {
                await Key.deleteOne({ key: inputKey });
                return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.ERROR).setTitle('❌ Thất Bại').setDescription('Mã key này đã hết hạn sử dụng!').setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
            }
            ...
            if (!row) return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.ERROR).setTitle('❌ Thất Bại').setDescription('Key không hợp lệ hoặc không thuộc sở hữu của bạn!').setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });

            const now = Date.now();
            if (row.expires_at !== 0 && now > row.expires_at) {
                await Key.deleteOne({ _id: row._id });
                return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.ERROR).setTitle('❌ Thất Bại').setDescription('Key của bạn đã hết hạn và bị xóa khỏi hệ thống!').setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
            }
            ...
            if (!tokenInput && (now - row.last_reset < cooldown)) {
                ...
                return interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.WARNING).setTitle('⏳ Đang Chờ Cooldown').setDescription(`Vui lòng đợi thêm **${hoursLeft} giờ** nữa để reset HWID hoặc dùng token cấp phép.`).setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
            }
            ...
            interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.INFO).setTitle('🔄 Reset HWID Thành Công').setDescription(`✅ Đã reset phần cứng thành công cho key:\n\n\`${inputKey}\`${tokenInput ? ' (Dùng Token)' : ''}`).setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
        }
        ...
        interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.ERROR).setTitle('❌ Lỗi Hệ Thống').setDescription('Đã xảy ra lỗi không mong muốn khi xử lý yêu cầu!').setThumbnail(userAvatar).setFooter(getFooterOptions()).setTimestamp()] });
    }
});
