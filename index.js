    ...
    if (commandName === 'setadmin') {
        ...
        db.get(`SELECT * FROM keys WHERE assigned_key = ?`, [toolKey], async (err, row) => {
            ...
            if (row.user_id) {
                try {
                    ...
                    await memberUser.send(`❗ **Thông báo từ hệ thống:** Key của bạn (\`${toolKey}\`) đã bị xóa bởi owner. Lý do: Bạn đã vi phạm hoặc owner đã xóa nhầm, hãy liên hệ để biết lý do và hoàn lại key.`);
                } catch (e) {}
            }
        ...
        db.get(`SELECT * FROM keys WHERE assigned_key = ? AND user_id = ?`, [inputKey, userId], (err, row) => {
            ...
            if (!tokenInput && (now - row.last_reset < cooldown)) {
                ...
                return interaction.reply({ content: `⏳ **Chờ thời gian cooldown:** Bạn cần đợi thêm **${timeLeft} giờ** nữa mới được reset HWID (hoặc sử dụng Token reset để bỏ qua).`, ephemeral: true });
            }
            ...
            rows.forEach((r, index) => {
                ...
                if (r.last_reset > 0) {
                    ...
                    if (timeSinceReset < cooldown) {
                        ...
                        resetStatus = `⏳ Reset HWID trong vòng ${hoursLeft} giờ tới kể từ lúc reset`;
                    }
                }
