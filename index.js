    ...
    try {
        if (commandName === 'setadmin') {
            ...
            if (targetUser) {
                ...
                await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.INFO).setTitle('🔑 Token Reset HWID').setDescription(`${tokenStr}`).setThumbnail(userAvatar).setFooter(getFooterOptions())] });
            }
        }
